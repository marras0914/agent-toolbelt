...
async function handler(input: Input) {
  ...
  const dataContext = [
    ...
    incomeRows.length > 0 ? `\nFinancials:\n${incomeRows.join("\n")}` : "",
    fh.revenueGrowth3Y != null ? `3Y Revenue CAGR: ${(fh.revenueGrowth3Y * 100).toFixed(1)}%` : "",
    `\nValuation:`,
    km.peRatioTTM != null ? `  P/E (TTM): ${Number(km.peRatioTTM).toFixed(1)}x` : fh.peNormalizedAnnual != null ? `  P/E: ${Number(fh.peNormalizedAnnual).toFixed(1)}x` : "",
    km.priceToSalesRatioTTM != null ? `  P/S: ${Number(km.priceToSalesRatioTTM).toFixed(1)}x` : "",
    km.freeCashFlowYieldTTM != null ? `  FCF Yield: ${(Number(km.freeCashFlowYieldTTM) * 100).toFixed(1)}%` : "",
    km.debtToEquityTTM != null ? `  Debt/Equity: ${Number(km.debtToEquityTTM).toFixed(2)}` : "",

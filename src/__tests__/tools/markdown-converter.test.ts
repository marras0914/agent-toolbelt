...
describe("markdown-converter", () => {
  describe("HTML to Markdown", () => {
    it("converts h1 to # heading", async () => {
      const r = await handler({ content: "<h1>Hello</h1>", from: "html", to: "markdown" });
      expect(r.output).toContain("# Hello");
    });

    it("converts h2 to ## heading", async () => {
      const r = await handler({ content: "<h2>Section</h2>", from: "html", to: "markdown" });
      expect(r.output).toContain("## Section");
    });

    it("converts <strong> to bold", async () => {
      const r = await handler({ content: "<p>Hello <strong>world</strong></p>", from: "html", to: "markdown" });
      expect(r.output).toContain("**world**");
    });

    it("converts <em> to italic", async () => {
      const r = await handler({ content: "<p>Hello <em>world</em></p>", from: "html", to: "markdown" });
      expect(r.output).toMatch(/_world_|\*world\*/);
    });

    it("converts <a> to markdown link", async () => {
      const r = await handler({ content: '<a href="https://example.com">Click</a>', from: "html", to: "markdown" });
      expect(r.output).toContain("[Click](https://example.com)");
    });

    it("converts <ul><li> to bullet list", async () => {
      const r = await handler({ content: "<ul><li>One</li><li>Two</li></ul>", from: "html", to: "markdown" });
  ...
  describe("same format passthrough", () => {
    it("returns input unchanged when from === to", async () => {
      ...
      const r = await handler({ content, from: "html", to: "html" });
  ...
  describe("response shape", () => {
    it("includes from, to, inputLength, outputLength", async () => {
      const r = await handler({ content: "<h1>Hi</h1>", from: "html", to: "markdown" });
